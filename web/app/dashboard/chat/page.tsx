...
import { MessengerWindow } from '@/components/chat/messenger-window'
import { useChatRooms } from '@/hooks/use-chat-rooms'
import { MessageSquare, Shield, Activity, Users } from 'lucide-react'
import { z } from 'zod'
import { useSearchParams, useRouter } from 'next/navigation'

// ── Steel Cage: Isolation Boundary ──
...
export default function AdminMessengerPage() {
    ...
    // Validate the room ID from the URL (Zod Guard)
    const roomId = roomIdSchema.safeParse(rawRoomId).success ? rawRoomId : null
    
    // ── V2 Optimization: Top-Level App State ──
    const { rooms, isLoading, refresh } = useChatRooms()
    const activeRoom = rooms.find(r => r.id === roomId)
    const activeParticipant = activeRoom?.borrower || null

    // ── Tactical Handshake Effect ──
    useEffect(() => {
        if (borrowId && !roomId) {
            const establishRoom = async () => {
                const result = await getOrCreateRoom(parseInt(borrowId))
    ...
    return (
        <div className="flex bg-white h-[calc(100vh-140px)] 14in:h-[calc(100vh-160px)] xl:h-[calc(100vh-180px)] rounded-3xl shadow-2xl overflow-hidden border border-slate-200/50 select-none animate-in fade-in duration-500">
            {/* Master List (Sidebar) */}
            <ChatSidebar 
            ...
            <div className="flex-1 flex flex-col bg-slate-50/20">
                {roomId ? (
                    <MessengerWindow
                        key={roomId} // Force reactive unmount per room context
                        roomId={roomId}
