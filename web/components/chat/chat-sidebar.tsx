...
/**
 * Chat Sidebar (2026 Gold Standard)
 * Powered by PostgreSQL RPC engine. No N+1 queries. Zero Node.js latency.
 */
export function ChatSidebar({ selectedRoomId, onSelectRoom, rooms, isLoading, refresh }: ChatSidebarProps) {
        ...
        <div className="flex flex-col h-full border-r border-slate-200 w-80 bg-white select-none">
            ...
            <div className="p-4 border-b border-slate-100 bg-white/50 backdrop-blur-sm sticky top-0 z-10 space-y-3">
                <div className="flex items-center justify-between">
                    <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Messages</h2>
                    <button
                        onClick={refresh}
                ...
                {filteredRooms.length === 0 ? (
                    ...
                    filteredRooms.map((room) => (
                        ...
                        >
                            ...
                            {/* UNREAD BADGE - Rose Domain */}
                            {room.unread_count > 0 && selectedRoomId !== room.id && (
                                <div className="flex-shrink-0 flex items-center justify-center">
                                    <span className="flex h-5 min-w-[20px] px-1.5 items-center justify-center rounded-full bg-gradient-to-br from-rose-500 to-rose-600 text-[10px] font-bold text-white shadow-[0_4px_12px_rgba(244,63,94,0.4)] border-2 border-white/95">
