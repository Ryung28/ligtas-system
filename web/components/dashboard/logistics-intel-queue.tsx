                        ...
                        intel.map((item, i) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className="group flex items-center gap-4 py-4 px-2 hover:bg-slate-50/50 transition-colors cursor-default"
                                >
                                    ...
                                    <Link 
                                        href={
                                            item.category === 'OVERDUE' || item.category === 'LOGISTICS'
                                                ? `/dashboard/logs?search=${encodeURIComponent(item.metadata?.search_query || item.metadata?.borrower_name || item.metadata?.requester_name || '')}&highlight=true`
                                                : item.category === 'INVENTORY'
                                                ? `/dashboard/inventory?search=${encodeURIComponent(item.metadata?.search_query || item.metadata?.item_name || '')}&highlight=true`
                                                : config.href
                                        }
                                        className="shrink-0 flex items-center gap-1 text-[9px] font-black uppercase tracking-wider px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all shadow-sm bg-white"
                                    >
