...
/// Background effects for splash screen (particles, waves, gradients)
class SplashBackgroundEffects {
  SplashBackgroundEffects._();
  ...
  static CustomPaint buildSubtlePattern({required Color color}) {
    return CustomPaint(painter: _SubtleBackgroundPainter(color: color));
  }
}
...
class _WavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path1.moveTo(
      0,
      height * 0.85 + 25 * math.sin(waveAnimation * 2 * math.pi - math.pi / 2),
    );
    for (double i = 0; i <= width; i++) {
      path1.lineTo(
        ...
        height * 0.85 +
            25 *
                math.sin(
                  (i / width) * 3.5 * math.pi + waveAnimation * 2 * math.pi,
                ),
      );
    }
    ...
    path2.moveTo(
      0,
      height * 0.9 + 20 * math.sin(waveAnimation * 2 * math.pi + math.pi / 3),
    );
    for (double i = 0; i <= width; i++) {
      path2.lineTo(
        i,
